...
import type { Cart, Collection, Menu, Page, Product, Image } from "./types";
...
export async function getCollections(): Promise<Collection[]> {
  ...
  const wixCollections = [
    ...
    ...reshapeCollections(items).filter(
      (collection) => !collection.handle.startsWith("hidden") && !collection.handle.startsWith("all")
    ),
  ];
...
export async function getPages(): Promise<Page[]> {
  ...
  return pages.map((item) => ({
    ...
    reseauxSociaux: item.data!.reseauxSociaux,
